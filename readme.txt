...
Игрок Player
-имеет имя при инициализации name
-имеет модуль инстанса Deck



Дилер Dialer
-при инициализации имеет имя Дилер 
-имеет модуль инстанса Deck 
-имеет метод показывающий вместо карт звездочки close_cards

Стол Table
- создает колоду карт - массив cards
- тасует колоду случайным образом
- сдает карты по одной Игроку и Дилеру hand_card пока есть карты

карта Card
показывает значение карты
показывает масть
считает очки 
A = 1 или 11 если сумма больше 21
J , Q, K = 10

Банк Bank
 добавляет выигранную ставку increase_balance
 уменьшает проигранную ставку decrease_balance
 проверяет результат игры и увеличивает или уменьшает бак диллера и игрока result
 показывает результат игры result_show

модуль интстанса Deck
- имеет метод добавляющий карты при сдаче по одной в колоду игрока/дилера deck_add(card)
- метод показывающий карты в колоде игрока/дилера      open_cards
- метод считает сумму карт в колоде игрока/диллера     points
